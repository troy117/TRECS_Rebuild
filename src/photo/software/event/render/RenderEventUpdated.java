...
public class RenderEventUpdated implements Runnable
{
	...
	public RenderEventUpdated(EventGUI gui, ArrayList<EventStudent> students, String schoolPath, 
			String outputPath,int plan, int sort, RenderGUI render, boolean renderLabels, boolean includeIsland, boolean download)
	{
	...
	private ArrayList<EventStudent> createRenderList()
	{
		...
		for(EventStudent s:students)
		{
			if(plan==2)	//This is the dance package plan
			{
				if(s.order.contains("."))
				{
					...
					for(String o:t)
					{
						if(o.equals("1")) renderStudents.add(new EventStudent(s,"Package A"));
						else if(o.equals("2")) renderStudents.add(new EventStudent(s,"Package B"));
					}
				}
				else if(s.order.equals("1")) renderStudents.add(new EventStudent(s,"Package A"));
				else if(s.order.equals("2")) renderStudents.add(new EventStudent(s,"Package B"));
			}
			else if(plan==3)
			{
				if(s.order.contains("."))
				{
					...
					for(String o:t)
					{
						if(o.equals("1")) renderStudents.add(new EventStudent(s,"Package 1"));
						else if(o.equals("2")) renderStudents.add(new EventStudent(s,"Package 2"));
						else if(o.equals("3")) renderStudents.add(new EventStudent(s,"Package 3"));
					}
				}
				else if(s.order.equals("1")) renderStudents.add(new EventStudent(s,"Package 1"));
				else if(s.order.equals("2")) renderStudents.add(new EventStudent(s,"Package 2"));
				else if(s.order.equals("3")) renderStudents.add(new EventStudent(s,"Package 3"));
			}
			else	//This is for the 5x7 and wallet count
			{
				try	{orders = Integer.parseInt(s.order);}catch(NumberFormatException e){orders = 0;}
				for(int i = 0;i<orders;i++) renderStudents.add(new EventStudent(s,(i+1)+" of "+orders));
			}
		}
	...
	private void processImages()
	{
		...
		ArrayList<EventStudent> renderStudents = createRenderList();
		ArrayList<File> outputFiles = new ArrayList<File>();
		int count = 0, i=0;
		File f;
		for(EventStudent s:renderStudents)
		{
			try
			{	
				...
				else if(s.order.equals("Package 1"))	
				{
					create5x7Image(s,count,false);
					f = new File(outputPath+"\\"+job.job+"_"+String.format("%05d", count)+".jpg");
					ImageIO.write(cImage, "jpg", f);
				...
				else if(s.order.equals("Package 2"))
				{
					create5x7Image(s,count,false);
					f = new File(outputPath+"\\"+job.job+"_"+String.format("%05d", count)+".jpg");
					ImageIO.write(cImage, "jpg", f);
				...
				else if(s.order.equals("Package 3"))
				{
					create5x7Image(s,count,true);
					f = new File(outputPath+"\\"+job.job+"_"+String.format("%05d", count)+".jpg");
					ImageIO.write(cImage, "jpg", f);
					outputFiles.add(f);
					render.updateProgressBar((i*100)/renderStudents.size());
	...
	private void create5x7Image(EventStudent s, int count, boolean qrCode)
	{
		try
		{
			cG.setColor(Color.white);
			...
			if(qrCode) cG.drawImage(createQRImage("https://download.islandphotography.net/EVENT/"+job.job+"/"+s.image, 200), 1150, 2180, null);
			if(download)
			{
				//TODO Identify how I want the image to be encoded/stored, update the same for wallet.
				cG.drawString("Download from download.islandphotography.net/"+s.notes, 150, 2225);
	...
	private void createStudentWalletImage(EventStudent s, int count)
	{
		try
		{
			cG.setColor(Color.white);
