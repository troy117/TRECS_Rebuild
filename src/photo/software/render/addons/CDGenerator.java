...
public class CDGenerator 
{
	...
	ArrayList<String> exports;
	
	FontMetrics fm;
	
	public CDGenerator(ArrayList<RenderOrder> cds, String renderpath, RenderOrders orders)
	{
		this.cds = cds;
	...
	private void start()
	{
		for(int i=0;i<cds.size();i++)
		{
			if((plan.equals(""))||(!plan.equals(cds.get(i).plan)))
			{
				...
				try
				{
					planFile = new File("Templates\\PACKAGE_PLANS\\"+plan+"\\CDs.txt");
					if(planFile.exists())
					{
						initializeVariables();
						...
						while(planScan.hasNext())
						{
							...
							p = planLine.split("\t");
							if(p[0].contains("SourceFolder:")) exports.add(p[1]);
					...
					overlay = ImageIO.read(new File("Templates\\PACKAGE_PLANS\\"+plan+"\\CDCover.jpg"));
			...
			writeFinal(cds.get(i).last+"_"+cds.get(i).first+"__"+i+".jpg");
