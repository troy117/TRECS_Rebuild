...
{
	private ArrayList<RenderOrder> keychains;
	private String renderpath;
	private RenderOrders orders;
	
	BufferedImage background, overlay, sImage, sImage2,rImage;
	...
	String planLine;
	String[] p;
	int topX1, topY1, imgW1, topX2, topY2, imgW2, 
		nameX1, nameY1, maxW1, fontS1, activeSize1,
		nameX2, nameY2, maxW2, fontS2, activeSize2;
	boolean nameC1, nameC2, nameP1, nameP2, rotate;
	String color1, color2, font1, font2;
		
	public KeychainGenerator(ArrayList<RenderOrder> keychains, String renderpath, RenderOrders orders)
	{
	...
	private void start()
	{
		...
		{
			if(plan.equals("")||(!plan.equals(keychains.get(i).plan)))
			{
				plan = keychains.get(i).plan;
				if(overlay!=null) overlay.flush();
				overlay = null;
				try
				{
					planFile = new File("Templates\\PACKAGE_PLANS\\"+plan+"\\Keychain.txt");
					if(planFile.exists())
					{
						initializeVariables();
						...
						while(planScan.hasNext())
						{
							...
							p = planLine.split("\t");
							if(p[0].contains("Top Image1 X:")) topX1 = Integer.parseInt(p[1]);
							else if(p[0].contains("Top Image1 Y:")) topY1 = Integer.parseInt(p[1]);
							else if(p[0].contains("Image1 Width:")) imgW1 = Integer.parseInt(p[1]);
							else if(p[0].contains("Image Rotate (Y/N):")){
								if(p[1].contains("Y")) rotate = true;
								else rotate = false;
							}
					...
					overlay = ImageIO.read(new File("Templates\\PACKAGE_PLANS\\"+plan+"\\Keychain.png"));
				}catch(Exception e){JOptionPane.showMessageDialog(null, "Error Loading Keychain Template: "+e);return;}
			}
			...
			if(loadImage(keychains.get(i)))
			{
				positioner(keychains.get(i));
				writeFinal(i+"_"+keychains.get(i).homeroom+"_"+keychains.get(i).last+"_"+keychains.get(i).first+".jpg");
			}
			
			background.flush();
			background = null;
			orders.updateProgressBar((i*100)/keychains.size(), "Keychain: "+keychains.get(i).last+", "+keychains.get(i).first);
		}
	}
	...
	{
		if(rotate) sImage = rotate(sImage,true);
		resized = sImage.getScaledInstance(imgW1, -1, Image.SCALE_SMOOTH);
		bGraphics.drawImage(resized, topX1, topY1, null);
		
		if(imgW1!=imgW2) resized = sImage.getScaledInstance(imgW2, -1, Image.SCALE_SMOOTH);
	...
	private boolean loadImage(RenderOrder s)
	{
		...
		catch(Exception e){JOptionPane.showMessageDialog(null, "Error Loading Image for Keychain: "+e);return false;}
		return true;
	}
