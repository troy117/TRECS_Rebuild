...
{
	private ArrayList<RenderOrder> buttons;
	...
	public ButtonGenerator(ArrayList<RenderOrder> buttons, String renderpath, RenderOrders orders)
	{
	...
	private void start()
	{
		...
		{
			if(plan.equals("")||(!plan.equals(buttons.get(i).plan)))
			{
				plan = buttons.get(i).plan;
				if(overlay!=null) overlay.flush();
				overlay = null;
				try
				{
					planFile = new File("Templates\\PACKAGE_PLANS\\"+plan+"\\Button.txt");
					if(planFile.exists())
					{
						initializeVariables();
					...
					overlay = ImageIO.read(new File("Templates\\PACKAGE_PLANS\\"+plan+"\\Button.png"));
			...
			if(loadImage(buttons.get(i)))
			{
				positioner(buttons.get(i).first);
				writeFinal("Button"+i+"_"+buttons.get(i).last+"_"+buttons.get(i).first+".jpg");
			}
			background.flush();
			background = null;
			orders.updateProgressBar((i*100)/buttons.size(), "Button: "+buttons.get(i).last+", "+buttons.get(i).first);
		}
		overlay.flush();
		overlay = null;
	...
	private boolean loadImage(RenderOrder s)
	{
		...
		}catch(Exception e){JOptionPane.showMessageDialog(null, "Error Loading Student Image: "+e);return false;}
		return true;
	}
