...
public class MugGenerator 
{
	private ArrayList<RenderOrder> mugs;
	...
	public MugGenerator(ArrayList<RenderOrder> mugs, String renderpath, RenderOrders orders)
	{
		this.mugs = mugs;
	...
	private void start()
	{
		...
		{
			if((plan.equals(""))||(!plan.equals(mugs.get(i).plan)))
			{
				plan = mugs.get(i).plan;
				if(overlay!=null) overlay.flush();
				overlay = null;
				try
				{
					planFile = new File("Templates\\PACKAGE_PLANS\\"+plan+"\\Mug.txt");
					if(planFile.exists())
					{
						initializeVariables();
					...
					overlay = ImageIO.read(new File("Templates\\PACKAGE_PLANS\\"+plan+"\\Mug.png"));
			...
			if(loadImage(mugs.get(i)))
			{
				positioner(mugs.get(i).first);
				writeFinal(i+"_"+mugs.get(i).homeroom+"_"+mugs.get(i).last+"_"+mugs.get(i).first+".jpg");
			}
			background.flush();
			background = null;
			orders.updateProgressBar((i*100)/mugs.size(), "Mug: "+mugs.get(i).last+", "+mugs.get(i).first);
		}
	}
	...
	private boolean loadImage(RenderOrder s)
	{
		...
		catch(Exception e){JOptionPane.showMessageDialog(null, "Error Loading Image for Mug: "+e);return false;}
		return true;
	}
