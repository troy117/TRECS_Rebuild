...
public class RenderQREvent2 implements Runnable
{
	...
	public RenderQREvent2(QREventGUI gui, ArrayList<QRstudent> students, String schoolPath, 
			String outputPath,int plan, int sort, QRRenderGUI render, boolean renderLabels, boolean includeIsland)
	{
		...
		for(QRstudent s:students)
		{
			if(s.order.contains("."))
			{
				...
				for(String o:t)
				{
					...
					{
						renderStudents.add(new QRstudent(s,o));
					}
				}
			}
	...
	private void wallet5x7()
	{
		...
		for(QRstudent s:students)
		{
			...
			for(int i = 0;i<orders;i++)
			{
				...
				renderStudents.add(new QRstudent(s));
			}
		}
	...
	private void createStudentImage(QRstudent s)
	{
		try
		{
	...
	private void createStudentWalletImage(QRstudent s)
	{
		try
		{
			cG.setColor(Color.white);
	...
	private void createDanceImage(QRstudent s)
	{
		try
		{
			cG.setColor(Color.white);
