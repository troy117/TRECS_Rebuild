...
import photo.software.orders.SpringDataEntryScan;


public class SpringWatchScannerHotFolder extends Thread
{
	String schoolPath;
	SpringDataEntryGUI gui;
	Path hotFolder;
	
	Boolean valid = true, pause = false;
	WatchService watchService = null;
	
	public SpringWatchScannerHotFolder(String schoolPath, SpringDataEntryGUI gui)
	{
		this.schoolPath = schoolPath;
		this.gui = gui;
		
		hotFolder = Paths.get("ScannerFolder1");
	}
	public void setCaptureGUI(SpringDataEntryGUI gui)
	{
		this.gui = gui;
	}
	public boolean emptyHotFolder()
	{
		...
		{
			Arrays.sort(images);
			new SpringDataEntryScan(schoolPath,images[0].getName(),gui);
		}
		return false;
	}
	public void run()
		    ...
		    do 
		    {
		    	...
		    	for(WatchEvent<?> event : watchKey.pollEvents())
		    	{
		    		if (StandardWatchEventKinds.ENTRY_CREATE.equals(event.kind())) 
		    		{
		    			...
		    			{
		    				new SpringDataEntryScan(schoolPath,fileName,gui);	
		    			}
		    		}
		    	}
