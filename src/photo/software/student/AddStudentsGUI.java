...
public class AddStudentsGUI extends JInternalFrame implements ActionListener
{
	...
	public AddStudentsGUI(String schoolPath, StudentGUI studentGUI) 
	{
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(pressed == btnImportStudentList)
		{
			...
			int returnVal = chooser.showDialog(null, "ImportList");
		...
		{
			if(!previewed) JOptionPane.showMessageDialog(null,"Preview First");
			else
			{
				studentGUI.addStudents(importStudents.getStudentsToAdd());
				
				int blank = 0;
				try{blank = Integer.parseInt(textField_9.getText());}
					catch(Exception err){}
				if(blank>0) studentGUI.addBlank(blank);
				//new Logger(schoolPath,(blank+importStudents.getStudentsToAdd().size())+" students have been added to the database.");
				this.dispose();
			}
		}
